...
export default function CampaignsFilters({
  ...
  return (
    <PageFiltersBar
      search={search}
      searchLabel={MARKETING_COPY.filterLabels.search}
      searchPlaceholder={SEARCH_COPY.placeholders["/marketing"]}
      searchClearLabel={SEARCH_COPY.clear}
      onSearchChange={onSearchChange}
      onOpenSheet={onOpenSheet}
    >
      <FilterField label={MARKETING_COPY.filterLabels.status} className="w-40">
        {({ controlId }) => (
      ...
      <FilterField label={MARKETING_COPY.filterLabels.date} className="w-48">
        {({ controlId }) => (
