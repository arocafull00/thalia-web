...
function tintedSurfaceColors(hex: string): {
  ...
  return {
    lightColors: {
      main: hex,
      container: buildEventSurfaceColor(hex) ?? LIGHT_EVENT_SURFACE,
      onContainer: LIGHT_EVENT_INK,
    },
    darkColors: {
      main: hex,
      container:
        buildEventSurfaceColor(hex, DARK_EVENT_SURFACE) ?? DARK_EVENT_SURFACE,
...
function toCalendarEntry(employee: EmployeeCalendarSource): CalendarType {
  ...
  const { lightColors, darkColors } = tintedSurfaceColors(hex);
